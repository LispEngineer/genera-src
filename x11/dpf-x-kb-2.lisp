...
(define-keyboard-signature :mac-87-tenkeyless
  ...
  ; We want to remap escape to select, but have no practical way of doing this
  ; using the mapping below. What if we just tell it it's select in the first place?
 (61 :escape               :escape) ; Checked
; (61 :select :select) ; ESCAPE KEY
; (61 :f35 :f35) ; ESCAPE KEY
...
(define-keyboard-signature :mac-test
                           (:keycode-offset 0
                            :vendor-name "Macintosh Xquartz Tester")
)

(define-keyboard-mapping :mac-test ()
  (:f1  #\Escape #\Square)
  ; :escape doesn't seem to allow us to remap...
  (:escape #\Select #\Select)
  (#\Escape #\Select)
